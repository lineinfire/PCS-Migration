select * from mg_saving_recurring where saving_product_code = '020';
...
drop table mg_saving_recurring;

create table mg_saving_recurring
as
SELECT
        rownum AS SN                                 ,
        xyz.CLIENT_NAME                              ,
...
FROM
        (
                SELECT
                       ...
                       /*(
                                ...
                                FROM
                                        fn_saving_deposit 
                                WHERE
                                        account_no = SA.ACCOUNT_NO and deposit_date<= '01-Nov-17' ) -
                        (
                                SELECT
                                        SUM(withdraw_amount)
                                FROM
                                ...
                                WHERE
                                        account_no = SA.ACCOUNT_NO and withdraw_date<= '01-Nov-17' )
                        */
                         AS CURRENT_BALANCE,
                        SA.INTEREST_RATE                                               ,
                        (
                ...
                WHERE
                        SA.ACCOUNT_STATUS<>'C' and
                        sa.account_no in (
                        select distinct account_no from fn_saving_deposit where voucher_no is not null
                        ) --and
                        --sa.account_open_date <='01-Nov-17'
                ORDER BY
                        client_code) xyz,
